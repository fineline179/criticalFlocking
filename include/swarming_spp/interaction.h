...
class Interaction {
    public:
        ...
        virtual int get_neighbors(Agent* a0, int n_agents, Agent* ags, Agent** neis) { return 0; };

        virtual int get_neighbors(Agent* a0, int ag_number, int n_agents, Agent* ags, Agent** neis, double* posPairs) { return 0; };
        /* pure virtual, must be implemented */
...
/*
 * Balanced Topological interaction: an agent's neighbors
 * are the *k*-closest agents such any given pair of neighboring
 * agents is separated by at least an angle *thetaCrit*,
 * as measured from the source agent. This has the effect of
 * picking the *k* nearest neighbors that are uniformly
 * distributed in space around the source agent
 *
 */
class TopoBalanced: public Interaction {
public:
    // dd is a pointer to an array of velocities, with length n_agents*DIM,
    //  which is used to store the x,y,z values of the displacements of all
    //  n_agents from a given agent.
    TopoBalanced(int k, double mu, Geometry* g, double* dd);
    /* Copy A POINTER to the *k*
    * agents in *ags* closer to *a0* into *neis*.
    * Return the number of neighbors found
    * (should always be min(k,n_agents) ).
    */
    int get_neighbors(Agent* a0, int ag_num, int n_agents, Agent* ags, Agent** neis, 
                      double* pos_pairs);
    /* Return 1 if *a1* is one of the *k*
    * closest agents to *a0*.
    * WARNING: This requires a call to
